...
import {
  auditCouponReview,
  getCouponReview,
  type CouponReviewDetailResp,
  type CouponAuditIssueItem,
...
const form = reactive({
  reviewComment: '',
  issues: [] as (CouponAuditIssueItem & { fieldName: string })[],
})
...
// 打开问题弹框
const openIssueDialog = (field: CouponFormField) => {
...
// 编辑问题
const editIssue = (issue: CouponAuditIssueItem & { fieldName: string }) => {
...
const onOpen = async (id: string) => {
  ...
  try {
    const { data } = await getCouponReview(id)
...
const onBeforeOk = async () => {
  ...
  try {
    ...
    await auditCouponReview(dataDetail.value.id.toString(), reqData)
