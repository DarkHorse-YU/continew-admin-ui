<template>
  <a-drawer v-model:visible="visible" title="核销详情" :width="width >= 560 ? 560 : '100%'" :footer="false">
    <a-spin :loading="loading" style="width: 100%;">
      <!-- 状态头部卡片 -->
      <div class="status-header">
        <div class="status-header__left">
          <div class="status-header__title">{{ dataDetail?.activityName }}</div>
          <div class="status-header__id">券码：{{ dataDetail?.couponNo }}</div>
        </div>
      ...
      <div class="info-grid">
        <div class="info-item">
          <div class="info-item__label">券名称</div>
          <div class="info-item__value">{{ dataDetail?.templateName || '-' }}</div>
        </div>
        <div class="info-item">
          <div class="info-item__label">券类型</div>
          <div class="info-item__value">
            <span v-if="dataDetail?.couponType === 'CASH'">满减券</span>
            <span v-else-if="dataDetail?.couponType === 'DISCOUNT'">折扣券</span>
            <span v-else>-</span>
          </div>
        </div>
        <div class="info-item">
          <div class="info-item__label">优惠内容</div>
          <div class="info-item__value">
            <span v-if="dataDetail?.couponType === 'CASH' && dataDetail.discountAmount">
              ¥{{ dataDetail.discountAmount }}（满{{ dataDetail.thresholdAmount || 0 }}可用）
            </span>
            <span v-else-if="dataDetail?.couponType === 'DISCOUNT' && dataDetail.discountRate">
              {{ (dataDetail.discountRate * 10).toFixed(1) }}折
            </span>
            <span v-else>-</span>
          </div>
        </div>
        <div class="info-item">
          <div class="info-item__label">核销方式</div>
          <div class="info-item__value">
            <span v-if="dataDetail?.writeOffMode === 'QR_SCAN'">扫码核销</span>
            <span v-else-if="dataDetail?.writeOffMode === 'CODE_INPUT'">输码核销</span>
            <span v-else>-</span>
          </div>
        </div>
        <div class="info-item">
          <div class="info-item__label">核销时间</div>
          <div class="info-item__value">{{ dataDetail?.writeOffTime || '-' }}</div>
        </div>
        <div class="info-item">
          <div class="info-item__label">审核时间</div>
          <div class="info-item__value">{{ dataDetail?.auditTime || '-' }}</div>
        </div>
      </div>
            ...
            >
              ...
              <div class="field-card__value">
                ...
                <template v-else-if="field.fieldType === 'image'">
                  <div v-if="field.fileUrl || field.value" class="image-preview">
                    <a-image :src="field.fileUrl || field.value" width="100%" height="auto" fit="cover" />
                  </div>
                  <span v-else class="empty-value">暂无图片</span>
                </template>
                <!-- 文件类型 -->
                <template v-else-if="field.fieldType === 'file'">
                  <a v-if="field.fileUrl || field.value" :href="field.fileUrl || field.value" target="_blank" class="file-link">
                    <icon-file />
                    <span>点击查看文件</span>
                  </a>
      ...
      <!-- 审核意见 -->
      <template v-if="dataDetail?.auditComment">
        <div class="audit-comment-section">
          <div class="audit-comment-section__header">
            <icon-message />
            <span>审核意见</span>
          </div>
          <div class="audit-comment-section__content">{{ dataDetail.auditComment }}</div>
        </div>
      </template>
    </a-spin>
  </a-drawer>
...
import { type CouponReviewDetailResp, getCouponReview as getDetail } from '@/apis/marketing'

const { width } = useWindowSize()

const dataId = ref('')
const dataDetail = ref<CouponReviewDetailResp>()
const visible = ref(false)
const loading = ref(false)

// 状态文本
const statusText = computed(() => {
  return dataDetail.value?.statusDesc || dataDetail.value?.auditStatusDesc || '-'
})

// 状态样式类
const statusClass = computed(() => {
  const status = dataDetail.value?.auditStatus
  if (status === 'PENDING_AUDIT') return 'status-badge--pending'
